...
def draw_cube(ax: Axes, origin=(0, 0, 0), size=1, label=None, max_coords=(2, 2, 2)):
    ...
    for i, face in enumerate(faces):
        poly3d = Poly3DCollection([face], facecolors=colors[i], linewidths=1, edgecolors='k', alpha=0.6)  # 增加透明度
    ...
    if label is not None:
        ...
        def add_label(x_pos, y_pos, z_pos):
            # 创建带背景和轮廓的标签
            text = ax.text(x_pos, y_pos, z_pos, str(label),
                           color='black',
                           fontsize=8,
                           fontweight='bold',
                           ha='center',
                           va='center',
                           zorder=100,  # 确保标签总是在最上层
        ...
        # 前面和后面
        if x == 0:  # 前面
            add_label(center_x, y - label_offset, center_z)
        if x == x_max:  # 后面
            add_label(center_x, y + size + label_offset, center_z)

        # 左面和右面
        if y == 0:  # 左面
            add_label(x - label_offset, center_y, center_z)
        if y == y_max:  # 右面
            add_label(x + size + label_offset, center_y, center_z)

        # 底面和顶面
        if z == 0:  # 底面
            add_label(center_x, center_y, z - label_offset)
        if z == z_max:  # 顶面
            add_label(center_x, center_y, z + size + label_offset)
...
def draw_rubik_cube(ax: Axes, x=3, y=3, z=3):
    ...
    :param ax: 3D坐标系
    :param x: x轴方向的小立方体数:列
    :param y: y轴方向的小立方体数:行
    :param z: z轴方向的小立方体数:层
    """
    counter = 0  # 用于标记序号
    # 计算最大坐标值（减1是因为坐标从0开始）
...
"""
可以正常运行，但表达方式不同，三维坐标系点(x,y,z)表示魔方的(列，行，层)，而不是pytorch的(层，行，列)。
"""

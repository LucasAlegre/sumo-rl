...
def draw_cube(ax: Axes, origin=(0, 0, 0), size=1, label=None):
    ...
    for i, face in enumerate(faces):
        poly3d = Poly3DCollection([face], facecolors=colors[i], linewidths=1, edgecolors='k', alpha=0.8)  # 略微降低不透明度
    ...
    if label is not None:
        ...
        # 前面
        if x == 0:
            ax.text(center_x, y - 0.1, center_z, str(label), color='black', fontsize=8, ha='center', va='center')
        # 左面
        if y == 0:
            ax.text(x - 0.1, center_y, center_z, str(label), color='black', fontsize=8, ha='center', va='center')
        # 底面
        if z == 0:
            ax.text(center_x, center_y, z - 0.1, str(label), color='black', fontsize=8, ha='center', va='center')

...
class ContinuousSumoEnv(gym.Env):
    ...
    def __init__(
        ...
        super(ContinuousSumoEnv, self).__init__()
        """Initialize the environment."""
        assert render_mode is None or render_mode in self.metadata["render_modes"], "Invalid render mode."
        self.render_mode = render_mode
        self.virtual_display = virtual_display
        ...
        self.ts_ids = list(conn.trafficlight.getIDList())
        print("=====ContinuousSumoEnv ts_ids=", self.ts_ids)
        self.observation_class = observation_class

        if isinstance(self.reward_fn, dict):
            self.traffic_signals = {
                ts: ContinuousTrafficSignal(self, ts, self.delta_time, self.yellow_time, self.min_green, self.max_green,
                                            self.begin_time, self.reward_fn[ts], conn, )
                for ts in self.reward_fn.keys()
            }
        else:
    ...
    def step(self, action: Union[dict, np.ndarray]):
        """Apply the action(s) and then step the simulation for delta_time seconds.
    ...
    def _apply_actions(self, actions):
        ...
        Args:
            actions: 如果是单智能体, actions连续型动作空间中的值，如[0.53484094 0.49741495 0.49995264 0.523257],
                     必须转换为离散型动作空间中的值，如[1,0,0,0]，其中1所在的动作就是下一步的动作。self.num_green_phases (next green phase)
                     如果是多智能体, actions是一个字典dict, {ts_id : action,...},其中的action同单智能体。
        """
        ...
        if self.single_agent:
            ...
            discrete_action = continuous_to_discrete_actions(actions)
            for ts, action in discrete_action.items():
                if self.traffic_signals[ts].time_to_act:
                    self.traffic_signals[ts].set_next_phase(action)
    ...
    def _action_space(self):
        action_spaces = [self.traffic_signals[ts].action_space for ts in self.ts_ids]


def continuous_to_discrete_actions(actions):
    discrete_actions = {}

    for tl, continuous_action in actions.item():
        # 使用 argmax 找出最大值的索引

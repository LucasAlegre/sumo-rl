...
class LstmPolicy(ActorCriticPolicy):
    ...
    def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, n_lstm=256, reuse=False, layers=None,
        ...
        self.selfn_lstm * 2), dtype=np.float32)
        self.self

    def step(selfrministic=False):
        if deself
            rselfc_action, self._value, self.snew, self.neglogp],
             self self.states_ph: state, self.masks_ph: mask})
        else:self
...
class CnnPolicy(FeedForwardPolicy):
    ...
    :param n_batch: (int) The number of batch to run (n_envs * n_steps)
    :param reuse: (bool) If the policy is reusable or not
selffor the nature CNN feature extraction
self
self
selfenv, n_steps, n_batch, reuse=False, **_kwargs):
selface, ac_space, n_env, n_steps, n_batch, reuse,
selfraction="cnn", **_kwargs)
self

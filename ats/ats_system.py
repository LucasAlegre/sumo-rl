...
from ats.ats_traffic_env import AtsTrafficEnv
...
class TrafficControlSystem:
    def __init__(self, config_file: str):
        ...
        self.env = AtsTrafficEnv(**self.config.get("env_params"))
...
def main():
    ...
    config_path = os.path.join("ats/config", config_file)

    if not os.path.exists(config_path):
        print(f"Error: Configuration file {config_path} not found.")
        sys.exit(1)

    agent = TrafficControlSystem(config_path)
    agent.run()
